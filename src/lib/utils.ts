...
export function generateFaultDescription(propertiesOrArray: GeoJsonProperties | GeoJsonProperties[]): string {
  ...
  // Use faultzone if available, otherwise fall back to faultname
  let faultZone = properties.faultzone ? String(properties.faultzone) : (properties.faultname ? String(properties.faultname) : '');
  let faultName = properties.faultname ? String(properties.faultname) : '';
  let sectionName = properties.sectionname ? String(properties.sectionname) : '';
  let strandName = properties.strandname ? String(properties.strandname) : '';
  let mappedScale = properties.mappedscale ? String(properties.mappedscale) : '';
  let slipSense = properties.slipsense ? String(properties.slipsense) : '';
  let faultAge = properties.faultage ? String(properties.faultage) : '';
  let slipRate = properties.sliprate ? String(properties.sliprate) : '';
