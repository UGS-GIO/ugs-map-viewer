...
export class MapLibreLegend implements LegendProvider {
  ...
  private async getWMSLegend(sourceId: string, source: RasterSourceWithMetadata): Promise<RendererData> {
    try {
      ...
      // A WMS layer may bundle multiple GeoServer sublayers (comma-separated layer name).
      // GetLegendGraphic only accepts one layer per request, so fetch each and merge the rules.
      const layerNames = String(layerName).split(',').map(n => n.trim()).filter(Boolean);
      const previews: RegularLayerRenderer[] = [];

      for (const ln of layerNames) {
        const legendUrl = `${wmsUrl}?service=WMS&request=GetLegendGraphic&format=application/json&layer=${ln}&version=1.3.0`;
        ...
        if (!response.ok) {
          console.warn(`Failed to fetch WMS legend for ${ln}: ${response.status}`);
          continue;
        }
        ...
        if (!contentType?.includes('application/json')) {
          console.warn(`WMS legend returned non-JSON content for ${ln}: ${contentType}`);
          continue;
        }
