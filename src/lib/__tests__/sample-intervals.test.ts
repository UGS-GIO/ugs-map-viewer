...
describe('mergeSampleIntervals', () => {
  ...
  it('skips rows with no usable type value', () => {
    const rows = [
      { box_type: '', box_top_ft: 0, box_bottom_ft: 10 },
    ];
    expect(mergeSampleIntervals(rows, opts)).toEqual([]);
  });
  ...
  describe('notes roll-up', () => {
    ...
    it('includes notes_public when a single contributing row has a note', () => {
      const rows = [
        { box_type: 'Whole Core', box_top_ft: 0, box_bottom_ft: 10, notes_public: 'Fractured interval' },
      ];
      expect(mergeSampleIntervals(rows, notesOpts)).toEqual([
        { sample_type: 'Core', top_ft: 0, bottom_ft: 10, notes_public: 'Fractured interval' },
      ]);
    });

    it('omits notes_public entirely when no contributing row has a note', () => {
      const rows = [
        { box_type: 'Whole Core', box_top_ft: 0, box_bottom_ft: 10, notes_public: '' },
        { box_type: 'Whole Core', box_top_ft: 10, box_bottom_ft: 20, notes_public: null },
      ];
      const result = mergeSampleIntervals(rows, notesOpts);
      expect(result).toEqual([{ sample_type: 'Core', top_ft: 0, bottom_ft: 20 }]);
      expect(result[0]).not.toHaveProperty('notes_public');
    });

    it('dedupes and joins distinct notes from multiple merged rows', () => {
      const rows = [
        { box_type: 'Whole Core', box_top_ft: 0, box_bottom_ft: 10, notes_public: 'Note A' },
        { box_type: 'Slabs', box_top_ft: 10, box_bottom_ft: 20, notes_public: 'Note B' },
        { box_type: 'Whole Core', box_top_ft: 20, box_bottom_ft: 30, notes_public: 'Note A' },
      ];
      expect(mergeSampleIntervals(rows, notesOpts)).toEqual([
        { sample_type: 'Core', top_ft: 0, bottom_ft: 30, notes_public: 'Note A; Note B' },
      ]);
    });

    it('does not roll up notes when notesField is omitted', () => {
      const rows = [
        { box_type: 'Whole Core', box_top_ft: 0, box_bottom_ft: 10, notes_public: 'Should be ignored' },
      ];
